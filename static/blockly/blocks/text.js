...
Blockly.Blocks['text_concatenate'] = {
     ...
     */
    init: function () {
        this.setHelpUrl(Blockly.Msg.TEXT_CONCAT_HELPURL);
        this.setColour(Blockly.Blocks.texts.HUE);
        this.itemCount_ = 2;
        this.updateShape_();
        this.setOutput(true, 'String');
        this.setMutator(new Blockly.Mutator(['text_create_concat_item']));
        this.setTooltip(Blockly.Msg.TEXT_CONCAT_TOOLTIP);
    },
    /**
     * Create XML to represent number of text inputs.
     * @return {Element} XML storage element.
    ...
    decompose: function (workspace) {
        var containerBlock = Blockly.Block.obtain(workspace,
            'text_create_concat_container');
        containerBlock.initSvg();
        var connection = containerBlock.getInput('STACK').connection;
        for (var i = 0; i < this.itemCount_; i++) {
            var itemBlock = Blockly.Block.obtain(workspace, 'text_create_concat_item');
            itemBlock.initSvg();
            connection.connect(itemBlock.previousConnection);
            connection = itemBlock.nextConnection;
        }
    ...
    updateShape_: function () {
        ...
        if (this.itemCount_ == 0) {
            ...
            for (var i = 0; i < this.itemCount_; i++) {
                var input = this.appendValueInput('ADD' + i);
                if (i == 0) {
                    input.appendField(Blockly.Msg.TEXT_CONCAT_TITLE_CREATEWITH);
...
Blockly.Blocks['text_create_concat_container'] = {
    /**
     * Mutator block for container.
     * @this Blockly.Block
     */
    init: function () {
        this.setColour(Blockly.Blocks.texts.HUE);
        this.appendDummyInput()
            .appendField(Blockly.Msg.TEXT_CREATE_CONCAT_TITLE_CONCAT);
        this.appendStatementInput('STACK');
        this.setTooltip(Blockly.Msg.TEXT_CREATE_CONCAT_TOOLTIP);
        this.contextMenu = false;
    }
};

Blockly.Blocks['text_create_concat_item'] = {
    /**
     * Mutator block for add items.
     * @this Blockly.Block
     */
    init: function () {
        this.setColour(Blockly.Blocks.texts.HUE);
        this.appendDummyInput()
            .appendField(Blockly.Msg.TEXT_CREATE_CONCAT_ITEM_TITLE_ITEM);
        this.setPreviousStatement(true);
        this.setNextStatement(true);
        this.setTooltip(Blockly.Msg.TEXT_CREATE_CONCAT_ITEM_TOOLTIP);
        this.contextMenu = false;
    }
};
